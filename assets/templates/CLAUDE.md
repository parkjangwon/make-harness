# Claude Entry For make-harness

This file is part of the `make-harness` bootstrap template, not a completed project harness.
It is a thin synchronized projection of the shared project contract for Claude-compatible tools.
...
- If the user already has custom `CLAUDE.md` content in another project, preserve it and update only a harness-managed block.
- Keep `AGENTS.md` and `GEMINI.md` aligned as thin entry files that point back to the same project harness.
